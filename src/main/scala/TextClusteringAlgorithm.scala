...
class TextClusteringAlgorithm(val ap: AlgorithmParams) extends P2LAlgorithm[PreparedData, TCModel, Query, PredictedResult] {
  ...
  def train(sc: SparkContext, data: PreparedData): TCModel = {
    ...
    val art1 = data.docs.map(x=>(x._2.toLowerCase.replace(".","").split(" ").filter(k => !stopwords.contains(k)).map(normalizet).filter(_.trim.length>2).toSeq, (x._1._1, x._1._2, x._3) )).filter(_._1.size>0)
